...
static double benchmark_ms(auto lambda) {
 ...
 */
static constexpr array TEST_SENTENCES = { "The birch canoe slid on the smooth planks.",
...
static double mean(vector<double> series) {
    double sum = 0.0;
    for (double v : series) {
        sum += v;
    }
...
static void benchmark_printout(const char * arch_name, const vector<double> & generation_samples,
...
int main(int argc, const char ** argv) {
    ...
    const unique_ptr               runner{ runner_from_args(args, config) };
    vector<double>                 generation_samples;
    ...
    benchmark_printout(runner->loader.get().name, generation_samples, output_times);
    return 0;
}
