...
void add_common_args(arg_list & args) {
    ...
    args.add({ default_config.voice, "voice", "v",
...
generation_configuration parse_generation_config(const arg_list & args) {
    const generation_configuration config{ .use_cross_attn{ !args["no-cross-attn"] },
...
unique_ptr<tts_runner> runner_from_args(const arg_list & args, const generation_configuration & config) {
    return runner_from_file(args["model-path"], args["n-threads"], config, !args["use-metal"], !args["no-mmap"]);
}

void add_text_encoder_arg(arg_list & args) {
    args.add(
        { "", "text-encoder-path", "tep", "The local path of the text encoder gguf model for conditional generation" });
}
