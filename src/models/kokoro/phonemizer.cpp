...
std::string parse_voice_code(std::string voice_code) {
    ...
    bool                 search_by_lcc =
        !search_by_id && (voice_code.find("-") != std::string::npos || voice_code.find("_") != std::string::npos);
    ...
    while (espeak_voices[i] != nullptr) {
        ...
        if (search_by_lc) {
            ...
            } else if (has_prefix(language_part, voice_code) &&
                       (!primary_match || strlen(primary_match->identifier) > strlen(espeak_voices[i]->identifier))) {
                // prefer the smaller codes as longer codes typically refer to more specific locales
                primary_match = espeak_voices[i];
            } else {
                auto subparts = split(language_part, "-");
                if (subparts.size() > 1 && to_lower(subparts[1]) == voice_code &&
                    (!secondary_match || strlen(secondary_match->identifier) > strlen(espeak_voices[i]->identifier))) {
            ...
            if (has_prefix(identifier_parts[1], voice_code) &&
                (!primary_match || strlen(primary_match->identifier) > strlen(espeak_voices[i]->identifier))) {
                primary_match = espeak_voices[i];
            } else if (identifier_parts[0] == voice_code &&
                       (!secondary_match ||
                        strlen(secondary_match->identifier) > strlen(espeak_voices[i]->identifier))) {
                secondary_match = espeak_voices[i];
            }
        } else if (search_by_id && has_prefix(to_lower(espeak_voices[i]->identifier), voice_code) &&
                   (!primary_match || strlen(primary_match->identifier) > strlen(espeak_voices[i]->identifier))) {
            primary_match = espeak_voices[i];
        } else if (search_by_lcc && has_prefix(to_lower(identifier_parts[1]), voice_code) &&
                   (!primary_match || strlen(primary_match->identifier) > strlen(espeak_voices[i]->identifier))) {
            primary_match = espeak_voices[i];
        } else if (to_lower(espeak_voices[i]->name).find(voice_code) != std::string::npos &&
                   (!primary_match || strlen(primary_match->identifier) > strlen(espeak_voices[i]->identifier))) {
            primary_match = espeak_voices[i];
        }
        i++;
    ...
    fprintf(stdout,
            "Passed Espeak Voice Code '%s' doesn't directly match any known Espeak Voice IDs. Nearest match with name "
            "'%s' and id '%s' will be used instead.\n",
...
bool dictionary_response::is_match(corpus * text, conditions * flags) {
    ...
    return text->next(after_match.size()) == after_match && (!expects_to_be_proceeded_by_number || flags->was_number) &&
           (!not_at_clause_start || !flags->beginning_of_clause);
}
...
bool phonemizer::handle_numeric(corpus * text, std::string * output, conditions * flags) {
    ...
    if (!invalid_format) {
        if (large_number_separator != '\0' && decimal_separator == '\0' && count_since_break != 3) {
            invalid_format = true;
        } else if (count_since_break == 3 && last_break_char != '\0' && decimal_separator == '\0' &&
                   large_number_separator == '\0') {
            large_number_separator = last_break_char;
        } else if (count_since_break != 3 && last_break_char != '\0' && decimal_separator == '\0' &&
                   large_number_separator == '\0') {
...
bool phonemizer::handle_phonetic(corpus * text, std::string word, std::string * output, conditions * flags,
                                 size_t unaccented_size_difference) {
    if (flags->was_word && output->back() != ' ' && !flags->hyphenated) {
        output->append(" ");
    }
...
bool phonemizer::process_word(corpus * text, std::string * output, std::string word, conditions * flags,
                              bool has_accent) {
    ...
    } else if (can_be_roman_numeral(word) && is_all_upper(word) &&
               small_english_words.find(to_lower(word)) == small_english_words.end() &&
               handle_roman_numeral(text, output, flags)) {
        return true;
...
bool phonemizer::handle_punctuation(corpus * text, std::string next, std::string * output, conditions * flags) {
    ...
    if (next[0] == '.') {
        ...
        if (flags->was_word && (after == "s" || !is_alphabetic(after[0]))) {
            return handle_possession_plural(text, output, flags);
        } else if (flags->was_word &&
                   (CONTRACTION_PHONEMES.find(after) != CONTRACTION_PHONEMES.end() ||
                    CONTRACTION_PHONEMES.find(text->after(next.size(), 2)) != CONTRACTION_PHONEMES.end())) {
...
struct word_phonemizer * word_phonemizer_from_gguf(gguf_context * meta) {
    ...
    if (rule_keys_key == -1 || phoneme_key == -1) {
        TTS_ABORT(
            "Both 'phonemizer.rules.keys' and 'phonemizer.rules.phonemes' keys must be set in order to support "
            "phonemization.");
    }
...
struct phoneme_dictionary * phoneme_dictionary_from_gguf(gguf_context * meta) {
    ...
    if (keys_key == -1 || values_key == -1) {
        TTS_ABORT(
            "Both 'phonemizer.dictionary.keys' and 'phonemizer.dictionary.values' keys must be set in order to support "
            "phonemization.");
    }
