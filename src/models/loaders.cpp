...
unique_ptr<tts_runner> runner_from_file(const char * fname, int n_threads, const generation_configuration & config,
    ...
    if (use_mmap) {
        ...
        for (ggml_tensor & cur : ggml_tensor_iterator{ *weight_ctx }) {
            TTS_ASSERT(i < n);
            TTS_ASSERT(!strcmp(cur.name, gguf_get_tensor_name(&*meta_ctx, i)));
    ...
    unique_ptr   runner{ loader.from_file(meta_ctx, weight_ctx, n_threads, cpu_only, config) };
    ...
    TTS_ASSERT(&runner->loader.get() == &loader);
