...
void unigram_tokenizer::tokenize(const std::string & text, std::vector<uint32_t> & tokens) {
    ...
    if (dedupe_spaces) {
        // https://stackoverflow.com/a/8362145/10477326 . Avoid regex which is slow and 134.4 KiB of bloat
        normalized.erase(ranges::unique(normalized, [](char l, char r) { return l == ' ' && r == ' '; }).begin(),
                         end(normalized));
    }
